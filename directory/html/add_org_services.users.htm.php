    ...
    <div class="panel panel-info">
        <div class="panel-heading">
            <h3 class="panel-title">
                ADD Services Resources Training for <?= strtoupper($agencyData['agency_name']); ?>
        ...
        <div class="panel-body">
                    ...
                    <div class="col-sm-2">
                        ...
                        <select id="filter_category" class="form-control">
                            ...
                            if (empty($filter)) {
                                ...
                                echo "<option value='add_org_services?id={$_GET['id']}' style='color:red'>CLEAR FILTER</option>";
                                echo "<optgroup label='____________'></optgroup>";
                            }
                            ?>
                            <option value="add_org_services?id=<?= $_GET['id'] ?>&filter=SERVICE" <?= $selService ?>>SERVICE</option>
                            <option value="add_org_services?id=<?= $_GET['id'] ?>&filter=TRAINING" <?= $selTraining ?>>TRAINING</option>
                            <option value="add_org_services?id=<?= $_GET['id'] ?>&filter=RESOURCES" <?= $selResource ?>>RESOURCES</option>
                        </select>
                    </div>
                </div>
            </form>
            ...
            foreach ($data as $letter => $srtData) {
                ...
                foreach ($srtData as $row) {
                    $selectedService = ($_service->ChkIfServiceSelected($agency_id, $row['id'])) ? '<span class="glyphicon glyphicon-ok text-success" aria-hidden="true"></span>' : '';
...
<div class="modal fade" id="srtModal" tabindex="-1" role="dialog" aria-labelledby="srtModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="../_lib/agencyaction.php?action=<?= $_core->encode('AddAgencyServices2') ?>" class="form-horizontal">
...
<script>
    $(function () {
        ...
        $(".srt_type").on('click', function () {
            ...
            $.ajax({
                url: "../_lib/agencyajax.php?action=GetAgencyServices2&id=<?= $_GET['id'] ?>" + "&sid=" + srtTypeId,
