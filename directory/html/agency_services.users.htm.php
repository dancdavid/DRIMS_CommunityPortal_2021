<div class="col-sm-12 col-xs-12" style="padding-bottom: 30px;">
    <h4 class="text-danger" style="padding-bottom: 10px;">AGENCY LIST 

        <?php
        $_core = new core();
        $_agency = new agency();
        
        echo "<font class='text-primary'><span class='glyphicon glyphicon-arrow-right'></span> {$_agency->build_search_breadcrumb($_core->gpGet('sid'), $_core->gpGet('t'), $_core->gpGet('l'))}</font> <a href='agency_directory' class='btn btn-xs btn-danger'>Clear Search <span class='glyphicon glyphicon-remove-sign' aria-hidden='true'></span></a>";
        
        if ($_SESSION['user_type'] === 'ADMIN') {
            echo '<a href="add_agency" class="btn btn-xs btn-danger pull-right"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span> ADD NEW AGENCY</a><br><br>';
        }
        ?>

        <button data-toggle="modal" data-target="#searchServicesModal" class="btn btn-xs btn-warning pull-right" style="margin-top:-10px;">SEARCH SERVICES</button>
...
<div class="modal fade" id="searchServicesModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <form class='form-horizontal' action='../_lib/action.php?action=<?php echo $_core->encode('searchServices'); ?>' method='post'>
            <div class="modal-content">
...
<script type="text/javascript" src="../js/agencyListServices.js"></script>
